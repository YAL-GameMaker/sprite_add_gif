#define gif_preinit
// Generated at 2023-05-08 20:33:05 (399ms) for v1.4.1804+
/// @lint nullToAny true
// Feather disable all
...
globalvar g_gif_white1; /// @is {sprite}
g_gif_white1=-1;
...
var __break=false;
var _white1=g_gif_white1;
if(_white1==-1){
	var _ws=surface_create(1,1);
	surface_set_target(_ws);
	draw_clear(16777215);
	surface_reset_target();
	_white1=sprite_create_from_surface(_ws,0,0,surface_get_width(_ws),surface_get_height(_ws),false,false,0,0);
	surface_free(_ws);
	g_gif_white1=_white1;
}
var __g=0;
var __g1=_q[3];
while(__g<array_length_1d(__g1)){
	...
	switch(_block[0]){
		case 0:
			...
			repeat (_fHeight) {
				var _x=0;
				repeat (_fWidth) {
					var _col=_pxData[_ind++];
					if(_col!=_transparentIndex){
						var _c32=_colorTable[_col];
						draw_sprite_ext(_white1,0,_x,_y,1,1,0,_colorTable[_col],1);
					}
					++_x;
				}
				++_y;
